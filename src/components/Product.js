...
function Product() {
  ...
  const addProduct = (product) => {
    const result = cartItems.filter(item => item.id === product.id)
      if (product.quantity >= select) {
        if (result.length > 0) {
          setCartItems(cartItems.map(item => item.id === result[0].id ?
            { ...item, quantity: item.quantity + Number(select) } : { ...item }))
        } else {
  ...
  const decQuantity = (product) => {
    const result = cartItems.find(it => it.id === product.id)
    if (result.quantity === 1) {
      setCartItems(cartItems.filter(fi => fi.id !== product.id))
    } else {
  ...
  return (
    <Container className="shadow mb-5 p-2 rounded" >
      <Row>
              ...
              return <div key={i} className="m-2">
                ...
                <Card style={{ width: '1rem' }}>
                  ...
                  <Card.Body>
                    ...
                    <Card.Text>
                      {`Rs. ${product.price * select}`}
                    </Card.Text>
                    <select className="m-2" onChange={(e) => { handleChange(e, product) }} >
                      <option value="1">{`1kg-Rs.${product.price}`}</option>
        ...
        <Col xs={6} md={4}>
          <Cart cartItems={cartItems} decQuantity={decQuantity} incQuantity={addProduct} />
        </Col>
      </Row>
    </Container>
  )
